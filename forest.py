...
sql="select winprob,loseconc,homeformadj/100,awayformadj/100,coalesce(pctwin,0),coalesce(pctnotwin,0) from traingamepred"
#sql="select winprob,coalesce(pctwin,0) from traingamepred"
X=lq.getdata2(sql)
sql="select case ftr when 'H' then 20 when 'A' then 10 when 'D' then 0 end from traingamepred"
y=lq.getdata2(sql)
