...
def callproc(data,type):
    ...
    cur.execute('select checkres(%s::numeric[],%s::varchar)',[data[0:3,0].astype(float).tolist(),type])
...
print "Big home win"
sql="select winprob,round(homeformadj,2) hf ,awayformadj\
         from traingamepred where type='BH'"
arr1=getdata(sql,'BH')
callproc(arr1,'BH')
